...
class PostsKafkaResourceIT {
    ...
    void setup() {
        ...
        PostsKafkaResource kafkaResource = new PostsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/posts-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/posts-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
